...
当前版本：**v0.1.4**（`versionCode 4` / `applicationId com.readit.eink` / minSdk 19 / targetSdk 34）。
...
- **分辨率适配与点对点渲染**：首启检测分辨率与密度，按物理 dpi 分三档布局（sw400 / sw600）；设置页可切渲染模式（自动 / 平滑 / 锐利），锐利模式关闭抗锯齿并整像素吸附字体与边距，墨水屏 1-bit 面板上文字边缘不再发虚
- 配色纯黑白，禁用动画与过渡
...
- 书架长按菜单：**重命名 / 删除 / 属性书籍**，并一并迁移该书的阅读进度、编码记忆、强制导入标记；「属性」对话框显示阅读进度百分比、总体字数、章节数（如有）、加入书库时间（后台增量扫描各格式字数 / 章节 / 页数）
- 自定义书籍目录
